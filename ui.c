...
#define TRANS_BK_COLOR_BLACK RGB(0,0,0)
#define COLOR_TEXT			 RGB(132,112,255)
#define FIGHTER_HEIGHT		 140
#define FIGHTER_WIDTH		 300
...
#define COLOR_BULLET RGB(65,105,225)
	...
	LPARAM lParam) // 消息参数，不同的消息有不同的意义，详见MSDN中每个消息的文档
{
	...
	case WM_CREATE:
		...
		PlaySound("C:\\Users\\Administrator\\Documents\\Visual Studio 2015\\Projects\\flighterfight\\flighterfight\\Lana Del Rey - Young And Beautiful.wav",
...
LONG FightWindowPaint(HWND hwnd)
{
	...
	*******************************************************************************/
	...
	hFont = CreateFont(32, 0, 0, 0, FW_DONTCARE, 0, 0, 0, DEFAULT_CHARSET, OUT_OUTLINE_PRECIS,
		CLIP_DEFAULT_PRECIS, CLEARTYPE_QUALITY, FF_SCRIPT, TEXT("Comic Sans MS"));

	if (hOldFont = (HFONT)SelectObject(hdcMem, hFont))
	{
		CHAR szSourceInfo[1024];
		wsprintf(str_show, "          飞机大战︿(￣︶￣)︿        Flys Count: %d", num);
	...
	for (i = 0; i < num; i++)
	{
		...
		if (auto_fly->type == FLY_TYPE_BULLET)
		{
			/*Ellipse(hdcMem,
				...
				get_fly_x(auto_fly) + 4,
				get_fly_y(auto_fly) + 6);*/
			hbmpFire = LoadImage(NULL, "fireI.bmp",
				IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
			SelectObject(hdcBitmapSrc, hbmpFire);
			GetObject(hbmpEnemy, sizeof(BITMAP), &bmp);

			StretchBlt(hdcMem,
				get_fly_x(auto_fly) - FIRE_WIDTH / 2, get_fly_y(auto_fly),
		...
		else if (auto_fly->type == FLY_TYPE_ENEMY)
		{
			...
			{
				// todo 爆炸；
				hbmpEnemy = LoadImage(NULL, "fire.bmp",
					IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				SelectObject(hdcBitmapSrc, hbmpEnemy);
				GetObject(hbmpEnemy, sizeof(BITMAP), &bmp);

				StretchBlt(hdcMem,
					get_fly_x(auto_fly) - ENEMY_WIDTH / 2, get_fly_y(auto_fly),
			...
			else
			{
				/*Ellipse(hdcMem,
					...
					get_fly_x(auto_fly) + 20,
					get_fly_y(auto_fly) + 10);*/
				hbmpEnemy = LoadImage(NULL, "fight.bmp",
					IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
				SelectObject(hdcBitmapSrc, hbmpEnemy);
				GetObject(hbmpEnemy, sizeof(BITMAP), &bmp);
...
LONG BackgroundCreate(HWND hwnd)
{
	...
	MoveWindow(hwnd, 200, 100, MAX_X, MAX_Y, TRUE);
...
{
	//BITMAP bmp;
	hbmpFighter = LoadImage(NULL, "飞机.bmp",
		IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE);
...
LONG OnKeydown(HWND hwnd, UINT vk)
{
	...
	case ' ':
		...
		//PlaySound("C:\\Users\\Administrator\\Documents\\Visual Studio 2015\\Projects\\flighterfight\\flighterfight\\火焰音效.wav",
			//NULL, SND_FILENAME | SND_ASYNC);
		break;
	default:
		break;
